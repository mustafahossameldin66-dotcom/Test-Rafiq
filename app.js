...
async function bootCore(){
  ...
  if('serviceWorker'in navigator&&location.protocol.startsWith('http'))navigator.serviceWorker.register('./sw.js?v=82',{updateViaCache:'none'}).catch(()=>{});
...
(function(){
  ...
  function dialect(text){return String(text||'')}
...
async function startApp(){
  ...
  try{await window.RafiqV60?.boot?.()}catch(err){console.error('[Rafiq] legacy curriculum boot failed',err)}
  try{if(typeof bootCurriculum==='function')bootCurriculum()}catch(err){console.error('[Rafiq] curriculum boot failed',err)}
  try{if(typeof bootRelease==='function')bootRelease()}catch(err){console.error('[Rafiq] release boot failed',err)}
...
Object.assign(window,{startOceanSound,stopOceanSound,reviewEntry,closeModal,deleteEntry,deleteMistake,openRecorder,openStudy,openAyahStudy,mushafInit,switchView,openSpace,addRep});
