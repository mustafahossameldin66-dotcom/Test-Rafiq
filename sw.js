const CACHE='rafiq-v70-shell-v1';
const API_CACHE='rafiq-v70-api-v1';
...
self.addEventListener('fetch',event=>{
  ...
  event.respondWith((async()=>{
    ...
    try{const res=await fetch(req);const copy=res.clone();const cache=await caches.open(CACHE);cache.put(req,copy).catch(()=>{});return res}
